...
const DoubleLineChart: FC<IDoubleLineChart> = ({ title, firstLineData, secondLineData }) => {
  const options = {
    responsive: true,
    scales: {
      x: {
  ...
  const data = {
    labels: new Array(Math.max(firstLineData.data.length, secondLineData.data.length)).fill(''),
    datasets: [
      {
